...
TEST_CASE("UTF8Text can parse unicode text", "[utf8]") {
  using UTF8Text = muton::playground::llm::UTF8Text;
  SECTION("Simple ASCII text") {
    std::vector<std::string> expected_str{"T", "e", "s", "t"};
    std::vector<uint32_t> expected_cp{'T', 'e', 's', 't'};
    size_t i = 0;
    for (auto symbol : UTF8Text("Test")) {
